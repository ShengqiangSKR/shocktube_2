program shocktube
  ...
  !sulute
  real::p12
  real::rou(-5:605),u(-5:605),p(-5:605),e(-5:605)
  real::roue(-5:605),ue(-5:605),pe(-5:605),ee(-5:605)
  !solver
  real::q(3,-5:605),qold(3,-5:605),flux(3,-5:605)
  ...
  open (18, file='p.csv', status='replace')
  open (19, file='u.csv', status='replace')
  open (20, file='rho.csv', status='replace')
  ...
  do n=1,nlast
     ...
     do i=1,mx
        !write (18, *) x(i), ',', now, ',', rou(i)*(e(i)+u(i)*u(i)/2), ',', rou(i)*(e(i)+u(i)*u(i)*0.5)
        write (18, *) x(i), ',', now, ',', p(i), ',', pe(i)
        write (19, *) x(i), ',', now, ',', u(i), ',', ue(i)
        write (20, *) x(i), ',', now, ',', rou(i), ',', roue(i)
...
contains
  subroutine calflx(g,mx,q,ecp,flux)
    ...
    real,intent(in)::q(3,-5:605),g,ecp
    real,intent(out)::flux(3,-5:605)
    real::q1lt,q1rt,q2lt,q2rt,q3lt,q3rt
    real::dq1,dq2,dq3,rlt,ult,plt,hlt
    real::rrt,urt,prt,hrt
    real::ubar,hbar,abar
    real::ram(3,-5:605),veck(3,3,-5:605),alfa(3,-5:605)
  ...
  subroutine exact(x,roue,ue,pe,ee,mx,roul,rour,pr,pl,p12,g,rgas,tr,tl,ur,ul,now)
    ...
    integer::i
    integer,intent(in)::mx
    real,intent(in)::x(-5:605)
    real,intent(out)::roue(-5:605),ue(-5:605),pe(-5:605),ee(-5:605)
    real::tmp,ptmp
